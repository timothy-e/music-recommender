...
class LogisticMF:
    ...
    def train(self):
        ...
        """
        self.user_vecs = np.random.normal(size=(self.n_users, self.n_latent_factors)) # n x f
        self.song_vecs = np.random.normal(size=(self.n_songs, self.n_latent_factors)) # n x f

        self.user_biases = np.random.normal(size=(self.n_users, 1))
        self.song_biases = np.random.normal(size=(self.n_songs, 1))

        ddx_user_vec_sum = np.zeros(shape=(self.n_users, self.n_latent_factors))
        ddx_song_vec_sum = np.zeros(shape=(self.n_songs, self.n_latent_factors))
    ...
    def log_likelihood(self):
        ...
        likelihood -= 0.5 * self.l2_regulation * np.sum(np.square(self.user_vecs))
        likelihood -= 0.5 * self.l2_regulation * np.sum(np.square(self.song_vecs))
...
if __name__ == "__main__":
    ...
    lmf = LogisticMF(M, n_latent_factors=5, alpha=2, l2_regulation=1, gamma=0.5, iterations=5)
    lmf.train()
